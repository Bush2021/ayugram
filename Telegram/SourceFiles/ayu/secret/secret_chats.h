...
public:
	...
	void readInbox(
		...
		MsgId tillId,
		bool ignoreGhost);
	void readContents(not_null<HistoryItem*> item);
...
private:
	struct Chat {
		...
		std::vector<uint64> pendingFileRetries;
		...
		uint64 sendingRandomId = 0;
		FullMsgId sendingPlaceholder;
		bool historyChanged = false;
		bool historyWritable = true;
	...
	void replaceUnacked(not_null<Chat*> chat, uint64 randomId);
	...
	base::Timer _resendRetryTimer;
	base::Timer _ttlTimer;
	int32 _pendingQts = 0;
	bool _ackNeeded = false;
	bool _statesWritable = true;
