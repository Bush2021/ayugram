...
void Chats::sendPrepared(
	...
	crl::async([=] {
		...
		const auto key = Crypto::GenerateFileKey();
...
void Chats::readInbox(
	...
	for (auto &record : chat->records) {
		...
		if (record.ttl > 0 && !record.destroyAt) {
			record.destroyAt = now + record.ttl;
			changed = true;
		}
	}
	if (changed) {
...
void Chats::ackOutgoingUpTo(not_null<Chat*> chat, int32 hisInSeqNo) {
	...
	const auto from = std::remove_if(
		...
		[&](const UnackedMessage &message) {
			return (message.outSeqNo < hisInSeqNo);
		});
	if (from == end(unacked)) {
		return;
	}
	unacked.erase(from, end(unacked));
	saveStates();
}
...
void Chats::sendNext(not_null<Chat*> chat) {
	...
	if (queued->service) {
		...
		chat->state.services.erase(
			ranges::remove(
				chat->state.services,
				randomId,
				&PendingService::randomId),
			end(chat->state.services));
	}
	...
	const auto done = [=](const MTPmessages_SentEncryptedMessage &result) {
		...
		if (record) {
			auto sent = *record;
			sent.id = entry->state.nextMsgId++;
			sent.date = date ? date : sent.date;
			result.match([&](const MTPDmessages_sentEncryptedFile &data) {
				data.vfile().match([&](const MTPDencryptedFile &data) {
					sent.file = FileInfo(data);
			...
			removePlaceholder(placeholder);
			appendRecord(entry, std::move(sent));
		} else {
...
void Chats::applyAction(
	...
	action.match([&](const SecretTLDdecryptedMessageActionResend &data) {
		...
		const auto ttl = std::max(data.vttl_seconds().v, 0);
		chat->state.ttl = ttl;
...
void Chats::dropRecords(
	...
	for (const auto &record : ranges::make_subrange(from, end(records))) {
...
void Chats::readStates() {
	auto states = DeserializeChatStates(session().local().readSecretChats());
...
void Chats::writeStates() {
	...
	session().local().writeSecretChats(SerializeChatStates(states));
}

void Chats::readHistory(not_null<Chat*> chat) {
	auto records = DeserializeHistory(
		session().local().readSecretHistory(chat->peer->id));
	if (records.empty()) {
		return;
	}
	std::sort(
